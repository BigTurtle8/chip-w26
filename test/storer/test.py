...
async def test_project(dut):
    ...
    dut._log.info("Reset")
    dut.rst.value = 1
    dut.begin_store.value = 0
    dut.addr.value = 0
    dut.store_val.value = 0
    ...
    assert dut.mem_w_req.value == 0

    dut._log.info("Go through standard expected store cycle")
    # Go to next time step, where can write again
    await NextTimeStep()
    dut.begin_store.value = 1
    dut.addr.value = 8
    dut.store_val.value = 0x67

    assert dut.mem_w_req.value == 0
    await ClockCycles(dut.clk, 1) 
    await ReadOnly()
    assert dut.mem_w_req.value == 1
    assert dut.mem_addr.value == 8
    assert dut.mem_w_val.value == 0x67
    assert dut.store_done.value == 0

    dut._log.info("Even after many cycles, should stay steady without other inputs")
    await NextTimeStep()
    dut.begin_store.value = 0
    dut.addr.value = 4
    dut.store_val.value = 0x69
    ...
    dut._log.info("Receive done notice; should combinationally pass along")
    await NextTimeStep()
    dut.mem_w_done.value = 1

    await ReadOnly()
    assert dut.store_done.value == 1
