...
async def test_project(dut):
    ...
    dut._log.info("Reset")
    dut.rst.value = 1
    dut.begin_fetch.value = 0
    dut.pc_addr.value = 0
    dut.fetch_valid.value = 0
    dut.fetch_instr.value = 0
    ...
    assert dut.fetch_req.value == 0

    dut._log.info("Go through standard expected fetch cycle")
    # Go to next time step, where can write again
    await NextTimeStep()
    dut.begin_fetch.value = 1
    dut.pc_addr.value = 8

    assert dut.fetch_req.value == 0
    await ClockCycles(dut.clk, 1) 
    await ReadOnly()
    assert dut.fetch_req.value == 1
    assert dut.fetch_addr.value == 8
    ...
    dut._log.info("Receive instruction; should combinationally pass along")
    await NextTimeStep()
    dut.fetch_valid.value = 1
    dut.fetch_instr.value = 0xc150

    await ReadOnly()
    assert dut.fetch_done.value == 1
    assert dut.instr.value == 0xc150
